...
## Enumeration
Enumeration is the process of understanding our target(victim-machine). Here target has been provided by an IPv4 address.
### NMAP Scan

So, we first use a tool called [`NMap`](https://www.varonis.com/blog/nmap-commands/#:~:text=At%20its%20core,%20Nmap%20is,operating%20systems%20they%20are%20running.&text=Nmap%20was%20developed%20for%20enterprise,through%20thousands%20of%20connected%20devices.) to scan the IP address. To know which services are running and what vulnerabilities it might hold.
...
So we use enumerate on port 80 as it hosts a website.
...
Root Privilege Escalation is required to get the highest permissions to system.
A root user can do anything with system, including the system source files itself.

On Doing `sudo -l` (this commands lists all the )
```SH
...
# VOILA!!
  ...
  VOILA! We found the root flag which means we pwned the machine.
